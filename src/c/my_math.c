/*
 based on code from https://github.com/mehrmann/pebble-sunclock/blob/master/src/my_math.c
 */
#include "my_math.h"
...
float sind(float x) {
	return my_sin(x * 0.01745329);
}

float cosd(float x) {
	return my_cos(x * 0.01745329);
}
...
float atand(float x) {
	if (x > 0) {
		return (M_PI / 2)*(0.596227*x + x*x) / (1 + 2 * 0.596227*x + x*x) * 180 / M_PI;
	}
	return -(atand(-x));
}
